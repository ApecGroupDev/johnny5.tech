...
function doLogin() {
  if (document.getElementById("pw").value.trim().toLowerCase() === "apec") {
...
async function liveAnswer(q, hits) {
  ...
  try {
    ...
    const r = await fetch(gatewayBase() + "/api/ruby-queen/ask", {
      method: "POST",
...
async function testGateway() {
  ...
  s.innerHTML = "Testing…";
  try {
    const r = await fetch(base + "/api/ruby-queen/ask", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function logContrib(action, tech, title, id) {
  if (!liveMode()) return;
  try {
    fetch(gatewayBase() + "/api/ruby-queen/contrib", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: action,
...
async function loadLeader(p) {
  ...
  try {
    const r = await fetch(gatewayBase() + "/api/ruby-queen/contrib", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function exportLeader() {
  ...
  window.open(
    gatewayBase() + "/api/ruby-queen/contrib?export=1&key=Summerville",
    "_blank",
  );
}
