...
export function CadPreview() {
  return (
    <div className="w-full max-w-[190px] h-full flex flex-col justify-center relative font-mono text-[9px] text-white/50">
      <div className="absolute top-1 left-1 text-[7px] text-cyan-400/50">GRID: 1.0m · ORTHO</div>
      <div className="absolute top-1 right-1 text-[7px]">SCALE 1:150</div>
      
      <div className="relative border border-cyan-500/20 bg-cyan-950/[0.05] h-20 w-full rounded-md flex items-center justify-center overflow-hidden">
        ...
        <svg viewBox="0 0 160 60" className="w-full h-full p-2">
          {/* Ground */}
          <line x1="10" y1="52" x2="150" y2="52" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
          
          {/* Columns */}
          <rect x="35" y="24" width="6" height="28" fill="var(--card-accent)" fillOpacity="0.2" stroke="var(--card-accent)" strokeWidth="1" />
          <rect x="119" y="24" width="6" height="28" fill="var(--card-accent)" fillOpacity="0.2" stroke="var(--card-accent)" strokeWidth="1" />
          
          {/* Canopy Roof Box */}
          <rect x="20" y="8" width="120" height="16" rx="1.5" fill="var(--card-accent)" fillOpacity="0.3" stroke="var(--card-accent)" strokeWidth="1.5" />
          
          {/* Height Dimension */}
          <line x1="8" y1="8" x2="8" y2="52" stroke="rgba(255,255,255,0.25)" strokeWidth="0.75" />
          <path d="M 6 12 L 8 8 L 10 12 M 6 48 L 8 52 L 10 48" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="0.75" />
          <text x="6" y="32" transform="rotate(-90, 6, 32)" textAnchor="middle" fill="rgba(255,255,255,0.4)" style={{ fontSize: "5px" }}>h: 4.8m</text>
          
          {/* Width Dimension */}
          <line x1="20" y1="57" x2="140" y2="57" stroke="rgba(255,255,255,0.25)" strokeWidth="0.75" />
          <path d="M 24 55 L 20 57 L 24 59 M 136 55 L 140 57 L 136 59" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="0.75" />
          <text x="80" y="56" textAnchor="middle" fill="rgba(255,255,255,0.4)" style={{ fontSize: "5px" }}>w: 24.5m</text>
        </svg>
      </div>
...
export function RainPreview() {
  ...
  const colors = {
    go: { bg: "rgba(21, 128, 61, 0.12)", border: "rgba(21, 128, 61, 0.30)", text: "#10b981" },
    chance: { bg: "rgba(180, 83, 9, 0.12)", border: "rgba(180, 83, 9, 0.30)", text: "#f59e0b" },
    nogo: { bg: "rgba(185, 28, 28, 0.12)", border: "rgba(185, 28, 28, 0.30)", text: "#ef4444" },
  };
  return (
      ...
      {days.map((d, i) => {
        ...
        return (
          <div key={i} className="flex flex-col items-center gap-1">
            ...
            >
              <span className="text-[12px] font-bold leading-none">{d.pct}%</span>
...
export function DatabasePreview() {
  return (
    <div className="w-full max-w-[190px] h-full flex flex-col justify-center relative font-mono text-[9px]">
      ...
      <div className="border border-emerald-500/20 bg-emerald-950/[0.03] rounded-md p-2 space-y-1">
        <div className="flex justify-between items-center border-b border-white/5 pb-1">
          <span className="text-white/60 font-semibold">APEC SITE ID: #360</span>
          <span className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[6.5px] px-1 rounded font-bold">ACTIVE</span>
        </div>
...
export function MapPreview() {
  return (
    <div className="w-full max-w-[190px] h-full flex flex-col justify-center relative font-mono text-[9px] text-white/50">
      ...
      <div className="relative border border-violet-500/20 bg-violet-950/[0.05] h-20 w-full rounded-md flex items-center justify-center overflow-hidden">
        {/* Radar Sweeper */}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(139,92,246,0.03)_0%,transparent_60%)]" />
        <div className="absolute w-full h-full bg-gradient-to-tr from-transparent to-violet-500/5 animate-[spin_5s_linear_infinite]" />
        ...
        <div className="absolute top-[25%] left-[30%]">
          ...
          <span className="absolute left-3 top-[-3px] text-[6.5px] bg-black/85 px-1 py-0.5 rounded border border-white/10 text-white/90">CREW 1</span>
        </div>
        ...
        <div className="absolute bottom-[30%] right-[25%]">
          ...
          <span className="absolute left-2.5 top-[-4px] text-[6.5px] bg-black/85 px-1 py-0.5 rounded border border-white/10 text-white/70">SITE #204</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full max-w-[190px] h-full flex flex-col justify-center gap-1 font-mono text-[8.5px] relative">
      ...
      <div className="flex flex-col gap-0.5 self-start max-w-[90%] rounded-lg bg-black/60 border border-white/5 p-1.5 text-white/50 backdrop-blur-xs">
        <div className="text-[7px] font-bold text-rose-400/80">RUBY QUEEN SYSTEM</div>
        <div className="leading-tight text-[8px]">POS reset library search... Found reset manual inside peggy_support/gilbarco.pdf.</div>
      </div>
    </div>
  );
}
